...
if game.PlaceId == 4623386862 then
    local function GetMap()
        local GameFolder = worskpace:WaitForChild('GameFolder', 10)
        local Map = nil

        for i, v in GameFolder:GetDescendants() do
            if v.Name == 'PlayerSpawn' and v.Parent:FindFirstChild('PeppaSpawn') then
                Map = v.Parent.Parent.Parent
                break
            end
        end
    ...
    Tabs.TP:AddButton({
        Title = 'TP to MainSpawn',
        Description = "TP's you to the MainSpawn in Workspace",
        Callback = function()
            local GameFolder = worskpace:WaitForChild('GameFolder', 10)

            if GameFolder then
                local SpawnPoint = GameFolder:WaitForChild('MainSpawn', 20)

                if SpawnPoint and SpawnPoint.ClassName == 'SpawnLocation' then
                    local localCharacter = Players.LocalPlayer.Character or Player.LocalPlayer.CharacterAdded:Wait()



                    if not localCharacter then
                        error('Did not find "localCharacter"')
                    end

                    local localHRP = localCharacter:WaitForChild('HumanoidRootPart', 15)

                    if localHRP then
                        localHRP.CFrame = SpawnLocation.CFrame
                    end
                end
            end
        end
    ...
    Tabs.TP:AddButton({
        Title = 'TP to MapSpawn',
        Description = "TP to Map",
        Callback = function()
            local Map = GetMap()
            print(Map.Name)
            ...
            if Map then
                ...
                localHRP.CFrame = Map:WaitForChild('Spawns', 10):WaitForChild('PlayerSpawn', 5).CFrame
            end
        end
    })

    Tabs.TP:AddButton({
        Title = 'TP to PiggySpawn',
        Description = "TP to Map",
        Callback = function()
            local Map = GetMap()
            ...
            if Map and Map:FindFirstChild('Spawns') then
                ...
                localHRP.CFrame = Map.Spawns:WaitForChild('PeppaSpawn', 5).CFrame
            end
        end
    })
end
...
-- loadstring(game:HttpGet('https://raw.githubusercontent.com/SubnauticaLaserMain/GG/main/main23.lua', true))()
